...
class ActivityTest(HSTestCase):
    ...
    def setUp(self):
        ...
        self.activity = self.hsclient.get_activity(self.projectid)
    ...
    def test_post_and_reverse_get(self):
        ...
        self.activity.post(data1)
        self.activity.post(data2)

        # read them back - activity should be second request first (reverse chronological order)
        result = list(self.activity.get(count=20))
        self.assertEqual(len(result), 20)
        reconstructed = result[10:] + result[:10]
        self.assertEqual(orig_data, reconstructed)

    def test_filters(self):
        self.activity.post({'c': i} for i in xrange(10))
        r = list(self.activity.get(filter='["c", ">", [5]]', count=2))
