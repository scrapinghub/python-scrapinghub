...
def delete_frontier_slot(hsproject, hsfrontier_name):
    frontier = hsproject.frontier
    frontier.delete_slot(hsfrontier_name, TEST_FRONTIER_SLOT)
...
def test_add_read(hsproject, hsfrontier_name):
    frontier = hsproject.frontier

    fps = [{'fp': '/'}]
    frontier.add(hsfrontier_name, TEST_FRONTIER_SLOT, fps)
    fps = [{'fp': '/index.html'}, {'fp': '/index2.html'}]
    frontier.add(hsfrontier_name, TEST_FRONTIER_SLOT, fps)
    frontier.flush()

    urls = [_get_urls(batch) for batch
            in frontier.read(hsfrontier_name, TEST_FRONTIER_SLOT)]
    expected_urls = [[u'/', u'/index.html', u'/index2.html']]
    assert urls == expected_urls


def test_add_multiple_chunks(hsproject, hsfrontier_name):
    frontier = hsproject.frontier
    old_count = frontier.newcount

    batch_size = 50
    fps1 = [{'fp': '/index_%s.html' % fp} for fp in range(0, batch_size)]
    frontier.add(hsfrontier_name, TEST_FRONTIER_SLOT, fps1)

    fps2 = [{'fp': '/index_%s.html' % fp}
            for fp in range(batch_size, batch_size * 2)]
    frontier.add(hsfrontier_name, TEST_FRONTIER_SLOT, fps2)

    fps3 = [{'fp': '/index_%s.html' % fp}
            for fp in range(batch_size * 2, batch_size * 3)]
    frontier.add(hsfrontier_name, TEST_FRONTIER_SLOT, fps3)
    frontier.flush()
    ...
    # get first 100
    batches = list(frontier.read(hsfrontier_name, TEST_FRONTIER_SLOT,
                                 mincount=100))
    ...
    frontier.delete(hsfrontier_name, TEST_FRONTIER_SLOT, ids)
...
def test_add_big_chunk(hsproject, hsfrontier_name):
    frontier = hsproject.frontier
...
def test_add_extra_params(hsproject, hsfrontier_name):
    frontier = hsproject.frontier
