...
def initialize_hubstorage_logging():
    ...
    e = os.environ
    observer = HubStorageLogObserver(e['SCRAPY_PROJECT_ID'], e['SCRAPY_SPIDER'],
        e['SCRAPY_JOB'], level=level, url=url)
...
class HubStorageLogObserver(object):
    ...
    def __init__(self, project, spider, job, level=log.INFO, url='http://localhost:8002'):
        self.level = level
        self.errors_count = 0
